...
void proto_register_RtspOverWs(void);
void proto_reg_handoff_RtspOverWs(void);
...
dissect_websocket_data_frame(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, proto_tree* pl_tree, guint8 opcode, websocket_conv_t* websocket_conv, gboolean pmc _U_, gint raw_offset _U_)
{
    ...
    case WS_BINARY: /* Binary */
        call_dissector(rtsp_handle, tvb, pinfo, tree);
...
static int
dissect_RtspOverWs(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data)
{
...
void
proto_register_RtspOverWs(void)
{
    ...
    proto_websocket = proto_register_protocol("RtspOverWs",
        "rtspoverws", "rtspoverws");
     ...
     * function.
     */
    heur_subdissector_list = register_heur_dissector_list("rws", proto_websocket);

    port_subdissector_table = register_dissector_table("rws.port",
        "TCP port for protocols using WebSocket", proto_websocket, FT_UINT16, BASE_DEC);

    protocol_subdissector_table = register_dissector_table("rws.protocol",
    ...
    websocket_handle = register_dissector("RtspOverWs", dissect_RtspOverWs, proto_websocket);

    websocket_module = prefs_register_protocol(proto_websocket, proto_reg_handoff_RtspOverWs);
...
void
proto_reg_handoff_RtspOverWs(void)
{
    //dissector_add_string("http.upgrade", "rtspoverws", websocket_handle);

    dissector_add_uint_with_preference("rtspoverws",12554, websocket_handle);
    ...
    rtsp_handle = find_dissector_add_dependency("rtsp", proto_websocket);
